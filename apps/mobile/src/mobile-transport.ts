	...
	async mutate<T>(
		...
		if (deckMatch && method === "PUT") {
			return (await this.deckService.update(
				parseInt(deckMatch[1], 10),
				this.userId,
				data,
			)) as T;
		}
		if (deckMatch && method === "DELETE") {
			await this.deckService.delete(parseInt(deckMatch[1], 10), this.userId);
			return { ok: true } as T;
		}
		...
		if (noteMatch && method === "DELETE") {
			await this.noteService.delete(parseInt(noteMatch[1], 10), this.userId);
			return { ok: true } as T;
		}
		...
		if (ntMatch && method === "PUT") {
			return (await this.noteTypeService.update(
				parseInt(ntMatch[1], 10),
				this.userId,
				data,
			)) as T;
		}
		if (ntMatch && method === "DELETE") {
			await this.noteTypeService.delete(parseInt(ntMatch[1], 10), this.userId);
			return { ok: true } as T;
		}
